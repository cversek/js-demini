 ...
 *
 * Outputs:
 *   DEMINI_annotated.js  - Full source with annotation comments
 *   DEMINI_stats.json    - Statement classification summary
 *
 * Annotation format:
...
if (!inputPath) {
    ...
    console.error('');
    console.error('Usage: node demini-annotate.js <input.js> [output-dir]');
    console.error('');
    console.error('Arguments:');
    console.error('  input.js    JavaScript bundle to annotate (required)');
    console.error('  output-dir  Directory for output files (default: same dir as input)');
    process.exit(1);
}
...
function classifyNode(node) {
    ...
    if (node.type === 'ForStatement' || node.type === 'ForInStatement' || node.type === 'ForOfStatement') return 'FOR_STMT';
    if (node.type === 'WhileStatement' || node.type === 'DoWhileStatement') return 'WHILE_STMT';
...
for (let i = 0; i < ast.body.length; i++) {
    ...
    // Gaps contain whitespace, comments, or other non-statement content.
...
console.log(`Statement bytes:   ${stats.total_bytes_statements.toLocaleString()} bytes`);
console.log(`Gap bytes:         ${stats.total_bytes_gaps.toLocaleString()} bytes`);
console.log(`Accounted:         ${accountedBytes.toLocaleString()} bytes`);
console.log(`Match:             ${accountingMatch ? '✅ 100%' : '❌ MISMATCH'}`);
if (!accountingMatch) {
    console.log(`  DELTA: ${(code.length - accountedBytes).toLocaleString()} bytes unaccounted`);
}
console.log(`Annotation overhead: ${stats.annotation_bytes.toLocaleString()} bytes`);
console.log(`Output size:       ${output.length.toLocaleString()} bytes`);

// --- Write Outputs ---

const annotatedPath = path.join(outputDir, 'DEMINI_annotated.js');
fs.writeFileSync(annotatedPath, output);
console.log(`\nWrote: ${annotatedPath}`);

stats.byte_accounting_match = accountingMatch;
const statsPath = path.join(outputDir, 'DEMINI_stats.json');
...
for (const [cat, count] of sorted) {
    ...
    console.log(`  ${cat.padEnd(20)} ${String(count).padStart(5)} stmts (${pct}%)  ${catBytes.toLocaleString().padStart(12)} bytes (${bytePct}%)`);
}
