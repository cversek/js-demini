...
if (!subcommand || !["match", "propagate", "apply", "merge", "diff", "enrich-sourcemap", "align", "stats"].includes(subcommand)) {
  ...
  console.error("  align <matched.bkg> <tgt-split> <ref-split> [-o pairs.json] [--scope-tolerance N]");
  console.error("                                                       — Generate escope rename pairs");
...
// =====================================================================
// SUBCOMMAND: align — Generate escope rename pairs from matched BKG
// =====================================================================

if (subcommand === "align") {
  const bkgPath = process.argv[3];
  const tgtSplitDir = process.argv[4];
  const refSplitDir = process.argv[5];

  if (!bkgPath || !tgtSplitDir || !refSplitDir) {
    console.error("Usage: demini-bkg align <matched.bkg> <target-split-dir> <reference-split-dir> [-o pairs.json] [--scope-tolerance N]");
    process.exit(1);
  }
  ...
  const tolIdx = process.argv.indexOf("--scope-tolerance");
  const scopeTolerance = (tolIdx !== -1 && process.argv[tolIdx + 1])
    ? parseInt(process.argv[tolIdx + 1], 10)
    : 0;

  console.log("=== demini-bkg align ===");
  console.log(`BKG:       ${bkgPath}`);
  console.log(`Target:    ${tgtSplitDir}`);
  console.log(`Reference: ${refSplitDir}`);
  console.log(`Tolerance: ${scopeTolerance}`);
  console.log(`Output:    ${outputPath}`);
  console.log("");
  ...
  let escope;
  try {
    const escopeModule = await import("escope");
    escope = escopeModule.default || escopeModule;
    if (typeof escope.analyze !== "function") {
      console.error("Error: escope loaded but analyze() not found. Keys:", Object.keys(escope));
      process.exit(1);
    }
    console.log("escope loaded successfully");
  } catch (e) {
    console.error("Error: escope not installed. Run: npm install escope");
    console.error("  Details:", e.message);
    process.exit(1);
  }

  const bkg = readBkg(path.resolve(bkgPath));
  const resolvedTgt = path.resolve(tgtSplitDir);
  const resolvedRef = path.resolve(refSplitDir);

  // Build file indexes for both split dirs
  ...
  const tgtFiles = buildFileIndex(resolvedTgt);
  const refFiles = buildFileIndex(resolvedRef);

  // Reserved names: JS keywords + runtime globals
  const RESERVED = new Set([
    ...
    "default","extends","finally","function","debugger","arguments","instanceof","implements",
    "name","length","prototype","constructor","__proto__","exports","module","require",
    "undefined","NaN","Infinity"
  ]);

  // Load runtime globals if available
  const runtimeFile = path.join(resolvedTgt, "mod_0000_RUNTIME.js");
  if (fs.existsSync(runtimeFile)) {
    try {
      const rCode = fs.readFileSync(runtimeFile, "utf8");
      const rAst = acorn.parse(rCode, { ecmaVersion: "latest", sourceType: "module" });
      walk.full(rAst, n => { if (n.type === "Identifier") RESERVED.add(n.name); });
    } catch (e) { /* skip */ }
  }

  // Also load export pairs to avoid overwriting them
  const exportKeys = new Set();
  // (Export pairs are generated by a separate tool — we just avoid duplicating them)

  // Process each matched module pair
  const allPairs = [];
  let exactModules = 0, relaxedModules = 0, scopeMismatch = 0;

  for (const mod of bkg.modules) {
    if (!mod.semantic_name || mod.minified_name === "RUNTIME") continue;
    const tgtName = mod.minified_name;
    const refName = mod.semantic_name;

    if (!tgtFiles[tgtName] || !refFiles[refName]) continue;

    let tgtScope, refScope;
    try {
      const tgtCode = fs.readFileSync(tgtFiles[tgtName], "utf8");
      const refCode = fs.readFileSync(refFiles[refName], "utf8");
      const tgtAst = acorn.parse(tgtCode, { ecmaVersion: "latest", sourceType: "module" });
      const refAst = acorn.parse(refCode, { ecmaVersion: "latest", sourceType: "module" });
      tgtScope = escope.analyze(tgtAst, { ecmaVersion: 2022, sourceType: "module" });
      refScope = escope.analyze(refAst, { ecmaVersion: 2022, sourceType: "module" });
    } catch (e) {
      // Log first few errors for debugging
      if (scopeMismatch + exactModules + relaxedModules < 3) {
        console.error(`  Parse/analyze error for ${tgtName}→${refName}: ${e.message}`);
      }
      continue;
    }

    const scopeDiff = Math.abs(tgtScope.scopes.length - refScope.scopes.length);

    if (scopeDiff === 0) {
      // Exact match
      exactModules++;
      for (let si = 0; si < tgtScope.scopes.length; si++) {
        const tV = tgtScope.scopes[si].variables;
        const rV = refScope.scopes[si].variables;
        if (tV.length !== rV.length) continue;
        for (let vi = 0; vi < tV.length; vi++) {
          const t = tV[vi], r = rV[vi];
          if (t.name === r.name || r.name.length < 2 || RESERVED.has(r.name) || RESERVED.has(t.name)) continue;
          allPairs.push({ module: tgtName, minified: t.name, semantic: r.name });
        }
      }
    } else if (scopeDiff <= scopeTolerance) {
      // Relaxed match — align by position within scope count tolerance
      relaxedModules++;
      const minScopes = Math.min(tgtScope.scopes.length, refScope.scopes.length);
      for (let si = 0; si < minScopes; si++) {
        const tV = tgtScope.scopes[si].variables;
        const rV = refScope.scopes[si].variables;
        if (Math.abs(tV.length - rV.length) > scopeTolerance) continue;
        const minVars = Math.min(tV.length, rV.length);
        for (let vi = 0; vi < minVars; vi++) {
          const t = tV[vi], r = rV[vi];
          if (t.name === r.name || r.name.length < 2 || RESERVED.has(r.name) || RESERVED.has(t.name)) continue;
          allPairs.push({ module: tgtName, minified: t.name, semantic: r.name });
        }
  ...
  fs.writeFileSync(path.resolve(outputPath), JSON.stringify(allPairs, null, 2));

  console.log(`=== Alignment Results ===`);
  console.log(`Exact scope modules: ${exactModules}`);
  console.log(`Relaxed (±${scopeTolerance}): ${relaxedModules}`);
  console.log(`Scope mismatch (>${scopeTolerance}): ${scopeMismatch}`);
